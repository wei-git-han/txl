...
public interface TxlUserDao extends BaseDao<TxlUser> {
     ...
     * @param id 
     */
	@Select("select * from TXL_USER where ORGANID = #{id} and ISDELETE=0 order by orderid")
     ...
     * @param id 
     */
	@Select("select USERID,FULLNAME, ACCOUNT, PASSWORD, SEX, ORGANID,(SELECT ORGANNAME FROM TXL_ORGAN b WHERE b.ORGANID=a.ORGANID and a.ISDELETE=0) as ORGANNAME,ORDERID, DN, ISDELETE, CA, ISMANAGER, TOKENID, SPID, SN, IP, STARTDATE, ENDDATE, USERUUID, USEREMAIL, SECLEVEL, FAILEDLOGINCOUNT, EDITPWDTIME, MOBILE, TIMESTAMP, TYPE, POST, TELEPHONE, ADDRESS,PYNAME from TXL_USER a where ISDELETE=0 and FULLNAME like '%'||#{1}||'%' or ORGANNAME like '%'||#{1}||'%' or POST like '%'||#{1}||'%' or TELEPHONE like '%'||#{1}||'%' or MOBILE like '%'||#{1}||'%' or ADDRESS like '%'||#{1}||'%' or GETPY(FULLNAME) like upper(#{1}||'%') order by orderid")
     ...
     * @param id 
     */
	@Select("select * from TXL_USER where ORGANID = #{0} and ISDELETE=0 and (FULLNAME like '%'||#{1}||'%' or ORGANNAME like '%'||#{1}||'%' or POST like '%'||#{1}||'%' or TELEPHONE like '%'||#{1}||'%' or MOBILE like '%'||#{1}||'%' or ADDRESS like '%'||#{1}||'%' or GETPY(FULLNAME) like upper(#{1}||'%')) order by orderid")
